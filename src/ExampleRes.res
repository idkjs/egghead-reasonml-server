...
module HelloJson = {
  ...
  let endpoint = Serbet.jsonEndpoint({
    verb: POST,
    path: "/json",
    ...
    handler: (body, _req) => \"@@"(async, {greeting: "Hello " ++ body.name}),
  })
}
...
module HelloQuery = {
  ...
  let endpoint = Serbet.endpoint({
    verb: GET,
    path: "/hello/query",
    // called with `http://localhost:3110/hello/query?name=Boss`
    handler: req =>
      %Async({
        let query = req.requireQuery(query_decode)
...
module AuthorizedHello = {
  ...
  let authorize = req =>
    %Async({
      let authHeader = req.requireHeader("authorization")
      switch authHeader {
...
let app = Serbet.application(~port=3110, list{hello, HelloJson.endpoint, HelloQuery.endpoint})
